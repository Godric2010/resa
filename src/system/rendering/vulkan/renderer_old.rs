...
use ash::vk::{AccessFlags, AttachmentDescription, AttachmentLoadOp, AttachmentReference, AttachmentStoreOp, Bool32, CommandBuffer, CommandBufferAllocateInfo, CommandBufferBeginInfo, CommandBufferLevel, CommandBufferResetFlags, CommandBufferUsageFlags, CommandPool, CommandPoolCreateFlags, CommandPoolCreateInfo, ComponentMapping, ComponentSwizzle, DebugUtilsMessageSeverityFlagsEXT, DebugUtilsMessageTypeFlagsEXT, DebugUtilsMessengerCallbackDataEXT, DebugUtilsMessengerEXT, DependencyFlags, DeviceMemory, Extent2D, Fence, FenceCreateFlags, FenceCreateInfo, Format, Framebuffer, FramebufferCreateInfo, Image, ImageAspectFlags, ImageCreateInfo, ImageLayout, ImageMemoryBarrier, ImageSubresourceRange, ImageTiling, ImageType, ImageUsageFlags, ImageView, ImageViewCreateInfo, ImageViewType, MemoryAllocateInfo, MemoryPropertyFlags, MemoryRequirements, PhysicalDeviceMemoryProperties, PipelineBindPoint, PipelineStageFlags, Queue, RenderPass, RenderPassBeginInfo, RenderPassBeginInfoBuilder, RenderPassCreateInfo, SampleCountFlags, Semaphore, SemaphoreCreateInfo, SharingMode, SubmitInfo, SUBPASS_EXTERNAL, SubpassDependency, SubpassDescription, SurfaceFormatKHR, SurfaceKHR};
...
impl<'a> VkRenderer {
    pub fn init(window: &Window) -> Self {
        ...
        let render_pass = VkRenderer::create_vk_renderpass(&vk_device.logical_device, &vk_swapchain.surface_format);

        let present_image_copy = present_image_views.clone();
        let framebuffers = VkRenderer::create_vk_framebuffers(&vk_device.logical_device, present_image_copy, &depth_image_view, &render_pass, &vk_swapchain.surface_resolution);
    ...
    pub fn get_next_present_index(&self) -> u32 {
        let mut present_index:u32;
            unsafe {
            let result:(u32, bool) = self.vk_swapchain.swapchain_loader
                .acquire_next_image(self.vk_swapchain.vk_swapchain, u64::MAX, self.present_complete_semaphore, Fence::null())
                .expect("Acquire next image failed");
            present_index = result.0;
        };
        present_index
    }

    pub fn get_render_pass_begin_info(&self) -> RenderPassBeginInfoBuilder{
       let begin_info = RenderPassBeginInfo::builder()
            .render_pass(self.render_pass)
            .framebuffer(self.framebuffers[0])
            .render_area(self.vk_swapchain.surface_resolution.into());
        begin_info
    }

    pub fn get_draw_command_buffer(&self) -> CommandBuffer{
    ...
    pub fn record_submit_buffer<F: FnOnce(&Device, CommandBuffer)>(device: &Device,
    ...
    fn create_vk_renderpass(device: &Device, surface_format: &SurfaceFormatKHR) -> RenderPass {
        let renderpass_attachments: [AttachmentDescription; 2] = [
        ...
        let depth_attachment_ref = AttachmentReference { attachment: 1, layout: ImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL };
        ...
        let renderpass = unsafe { device.create_render_pass(&renderpass_create_info, None).expect("Render pass creation failed!") };
        renderpass
    }

    fn create_vk_framebuffers(device: &Device, present_image_views: Vec<ImageView>, depth_image_view: &ImageView, render_pass: &RenderPass, surface_res: &Extent2D) -> Vec<Framebuffer> {
        let mut framebuffers;

        unsafe {
            framebuffers = present_image_views.iter()
                .map(|&present_image_view| {
                    let framebuffer_attachments = [present_image_view, *depth_image_view];
                    let framebuffer_create_info = FramebufferCreateInfo::builder()
                        .render_pass(*render_pass)
                        .attachments(&framebuffer_attachments)
                        .width(surface_res.width)
                        .height(surface_res.height)
                        .layers(1);
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            for framebuffer in &self.framebuffers {
                self.vk_device.logical_device.destroy_framebuffer(*framebuffer, None);
            }
            self.vk_device.logical_device.destroy_render_pass(self.render_pass, None);
